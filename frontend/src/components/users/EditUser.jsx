import '../../assets/components/users/EditUser.css'
...
function EditUser() {
    const webTitle = 'แก้ไขข้อมูลบัญชีผู้ใช้';
    ...
    const fetchUser = async () => {
        try {
            ...
            if (user) {
                const { employee_id, employee_name, position, username, email, organization, section, role } = user;
                ...
                throw new Error('เกิดข้อผิดพลาดในการดึงข้อมูลบัญชีผู้ใช้');
            }
        } catch (error) {
    ...
    const updateUser = async (e) => {
        ...
        if (!employeeID || !employeeName || !position || !username || !email || !selectedOrganization || !selectedSection || !selectedRole) {
            Swal.fire({
                icon: 'warning',
        ...
        await AxiosInstance.post(`/users/${id}`, formData).then(({data}) => {
            Swal.fire({
                icon: "success",
                        ...
                        <Card className="mb-5">
                            <Card.Body>
                                <h2 className="responsive-header card-title">
                                    <FaUserEdit size={30} className="m-3" />
                                    ...
                                    <Row>
                                        <Col md={12}>
                                            <Form.Group controlId="UserSection" hidden={!selectedOrganization}>
                                                ...
                                                <Form.Control as="select" value={selectedSection} onChange={handleSectionChange} isInvalid={validationError.hasOwnProperty('section')} >
